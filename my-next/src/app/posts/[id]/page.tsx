  ...
  useEffect(() => {
      const fetcher = async () => {
        ...
        const res = await fetch(
          `https://8hcgph0mr1.microcms.io/api/v1/posts${id}`,
          {
            headers: {
              'X-MICROCMS-API-KEY': 'lfwYdFwzijXQvN4q6cRwNMIls1HfVdDrjPsf',
            },
          },
        )
        ...
        <div className="px-5">
          <div className="flex justify-between">
            ...
            {post.categories && (<ul className="flex font-semibold text-sm">{post.categories.map((c)=>
              <li className="border border-blue-500 text-blue-500 rounded-md mr-1.5 px-1.5 py-0.5">{c.name}</li>)}</ul>)}
            </div>
